...
static void update_and_draw(void* arg)
{
    State* state = (State*)arg;

    Camera camera            = state->camera;
    bool* used_centroids     = state->used_centroids;
    Color* color_picker      = state->color_picker;
    const char** color_names = state->color_names;
    Color default_color      = state->default_color;
    Points normal_points     = state->normal_points;
    Points centroids         = state->centroids;
    float plane              = state->plane;
    int cur_cent             = state->cur_cent;
    ...
    if (cur_cent != NOT_FOUND) {
        DrawText(TextFormat("Current Centroid: %s", color_names[cur_cent]), 10,
                 10, 20, RED);
    } else {
...
static void loop(Camera camera)
{
    ...
    state = (State){camera,      used_centroids, color_picker,
                    color_names, default_color,  normal_points,
                    centroids,   plane,          cur_cent};
